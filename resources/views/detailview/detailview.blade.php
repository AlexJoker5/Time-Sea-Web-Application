...
$value = $index;
$viewed_watch = $watch[$value];

$description = $viewed_watch->shortDescription;
$name = $viewed_watch->serialName;
$company = $viewed_watch->company;
$madeIn = $viewed_watch->madeIn;
$releasedDate = $viewed_watch->releasedDate;
$price = $viewed_watch->price;
$img_path = $viewed_watch->image;
...
$result = $detailviewController->alreadyExist($user_id,$index);
...
<div class="body">
  <div class="">
    <div class="grid grid-rows-3 grid-flow-col gap-8 container-all">
      <div class="row-span-3 col-span-2 item-img">
          <div class="xzoom-container">
            <img class="xzoom" id="xzoom-default" src="{{asset($img_path)}}" xoriginal="{{asset($img_path)}}" />
            
          </div>   
          @if($result == true)
          <a onclick="chart(1)" href="{{ route('detailview.addToChart', ['userId' => $user_id,'watchId' => $index,'price' => $price ]) }}" type="button" class="text-white bg-green-400 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 chart-btn">
          ...
          @php echo('Remove from Cart') @endphp
          </a>
          @else
          <a onclick="chart(0)" href="{{ route('detailview.addToChart', ['userId' => $user_id,'watchId' => $index,'price' => $price ]) }}" type="button" class="text-white bg-green-400 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 chart-btn">
          <svg aria-hidden="true" class="w-5 h-5 mr-2 -ml-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M3 1a1 1 0 000 2h1.22l.305 1.222a.997.997 0 00.01.042l1.358 5.43-.893.892C3.74 11.846 4.632 14 6.414 14H15a1 1 0 000-2H6.414l1-1H14a1 1 0 00.894-.553l3-6A1 1 0 0017 3H6.28l-.31-1.243A1 1 0 005 1H3zM16 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM6.5 18a1.5 1.5 0 100-3 1.5 1.5 0 000 3z"></path></svg>
          
          @php echo('Add to Cart') @endphp
          </a>
          @endif
         <button onclick="buy()"type="button"  data-modal-target="defaultModal" data-modal-toggle="defaultModal" class="text-white bg-green-400 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 buy-btn">
              Buy Now
              <svg aria-hidden="true" class="w-5 h-5 ml-2 -mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
          </button>
      </div>
      
      <div class="row-span-3 col-span-2 item-detail">
      <div class="item-des">
        <h3 class="font-serif">
        {{$description}}
        </h3>
      ...
      <hr class="mt-1 mb-1">
        <p class="font-serif brand-name">
          Brand Name - {{$name}}
        </p>
        <p class="font-serif company-name">
          Company - {{$company}}
        </p>
        <p class="font-serif made-in">
          Made In - {{$madeIn}}
        </p>
        <p class="font-serif released-date">
        Released Date - {{$releasedDate}}
        </p>
        <p class="font-serif price">
        Price - ${{$price}}
        </p>
      </div>
    ...
    <div class="relative w-full max-w-2xl max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
          <!-- Modal header -->
            ...
            <!-- Form -->
          <form class="billing-form" action="{{ route('detailview.billing', ['userId'=>$user_id]) }}" method="POST">
          @csrf
